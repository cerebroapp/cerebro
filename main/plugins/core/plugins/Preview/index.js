...
export default class Preview extends Component {
  ...
  render() {
    ...
    const { name, version, repo } = plugin;
    const match = repo.match(/^.+github.com\/([^\/]+\/[^\/]+).*?/);
    return (
      <div className={styles.preview} key={name}>
        <h2>{name} ({plugin.version})</h2>
        <p>{plugin.description}</p>
        <KeyboardNav>
          <div className={styles.header}>
            {!installed && <ActionButton action={this.pluginAction(name, 'install')} text='Install' loadingText='Installing' />}
            {installed && <ActionButton action={this.pluginAction(name, 'uninstall')} text='Uninstall' loadingText='Uninstalling' />}
            {
              match &&
